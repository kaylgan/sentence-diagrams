...
let getOffset = (function(lookup = null, set = false, myMap = {}, addElmnt = null, addValues = []) {
  let offsetMap = {};
  let offset = 0;
  return function (lookupInner = lookup, setInner = set, mapInner = myMap, addElInner = addElmnt, addValInner = addValues) {
    // changed from addElInner = null, addValInner = []
...
let currentLineColor = (function(color = "black", set = false) {
  let lineColor = "black";
  return function(colorInner = color, setInner = set) {
    if (setInner == true) { lineColor = colorInner; }
    return lineColor;
  }
})();
...
let currentTextColor = (function(color = "black", set = false) {
  let txtColor = "black";
  return function(colorInner = color, setInner = set) {
    if (setInner == true) { txtColor = colorInner; }
    return txtColor;
  }
})();
  ...
  for (let i=0; i < navItems.length; i++) {
    navItems[i].addEventListener('click', function() {
      ...
      if (currentElement && (itemID != "showTT") && (itemID != "newLine") && (itemID != "platform")) {
        ...
        if (currentElement.textContent != initialText) {
          currentElement.textContent += "ing";
        }
        ...
        switch (itemID) {
          ...
          case ("gerund"):
            ...
            // only trim textContent if gerund button pressed, not unpressed
            if (currentElement.classList.contains("gerundBorder")) {
              let trimmed = currentElement.textContent.slice(0, -3);
              let ending = currentElement.textContent.slice(-3, currentElement.textContent.length+1);
              if (currentElement.textContent.length > 3 && ending == "ing") {
                currentElement.textContent = trimmed;
              }
            }
            break;
          default:
  ...
  return function(setInner = set, setValueInner = setValue) {
    if (setInner === true) {
      ...
      if (setValueInner) {
        ...
        getInitialText(true, currentElement.textContent);

        // if a new element is dragged, select it
        currentLineColor(currentElement.style.borderColor, true);
...
function arrowMovement() {
  ...
  switch (key) {
    ...
    case "=":
      if (currentElement.classList.contains("addedLine") || currentElement.classList.contains("addedPlatform")) {
        currentElement.style.height = currentElement.getBoundingClientRect().height + 1 + 'px';
        document.getElementById("lineSizeSlider").value = Math.round(currentElement.getBoundingClientRect().height);
        document.getElementById("rangeValue").textContent = Math.round(currentElement.getBoundingClientRect().height);
      } else {
        currentElement.style.width = currentElement.getBoundingClientRect().width + 1 + 'px';
        document.getElementById("lineSizeSlider").value = Math.round(currentElement.getBoundingClientRect().width);
        document.getElementById("rangeValue").textContent = Math.round(currentElement.getBoundingClientRect().width);
      }
      break;
    case "-":
      if (currentElement.classList.contains("addedLine") || currentElement.classList.contains("addedPlatform")) {
      ...
      break;
      case "1":
        currentElement.style.transformOrigin = 'left';
        if (currentElement.classList.contains("addedLine") || currentElement.classList.contains("addedPlatform")) {
      ...
      case "3":
        currentElement.style.transformOrigin = 'left';
        if (currentElement.classList.contains("addedLine") || currentElement.classList.contains("addedPlatform")) {
          currentElement.style.transform = 'rotate(180deg)';
        } else {
          currentElement.style.transform = 'rotate(270deg)';
        }
        break;
      case "4":
        currentElement.style.transformOrigin = 'left';
        if (currentElement.classList.contains("addedLine") || currentElement.classList.contains("addedPlatform")) {
          currentElement.style.transform = 'rotate(135deg)';
        } else {
          currentElement.style.transform = 'rotate(45deg)';
        }
        break;
      case "5":
        currentElement.style.transformOrigin = 'left';
...
function dragElement(elmnt) {
  ...
  function dragMouseDown(e) {
    ...
    if (elmnt.classList.contains("addedLine") || elmnt.classList.contains("addedPlatform")) {
      document.getElementById("lineSizeSlider").value = Math.round(elmnt.getBoundingClientRect().height);
      document.getElementById("rangeValue").textContent = Math.round(elmnt.getBoundingClientRect().height);
    } else {
      document.getElementById("lineSizeSlider").value = Math.round(elmnt.getBoundingClientRect().width);
      document.getElementById("rangeValue").textContent = Math.round(elmnt.getBoundingClientRect().width);
    }
...
function selectMultiple() {
  ...
  document.body.addEventListener('keydown', arrowMovementMult, true);
  ...
  window.addEventListener('dblclick', function() {
    ...
    document.body.removeEventListener('keydown', arrowMovementMult, true);
    // window.removeEventListener('mousedown', startRect, true);
    deselectAll();
  }, true);
  ...
  function endRect(e) {
    ...
    if (selectedElements.length > 0) {
      lastSelectedElement(true, selectedElements[0]);
    }
  }
  ...
  function arrowMovementMult() {
    if (selectedElements.length > 0) {
      let initialXOffsets = [];
      for (let i = 0; i < selectedElements.length; i++) {
        initialXOffsets.push(selectedElements[i].offsetLeft - selectedElements[0].offsetLeft);
      }

      let initialYOffsets = [];
      for (let i = 0; i < selectedElements.length; i++) {
        initialYOffsets.push(selectedElements[i].offsetTop - selectedElements[0].offsetTop);
      }
      ...
      switch (key) {
        case "ArrowLeft":
          ...
          for (let i = 1; i < selectedElements.length; i++) {
            selectedElements[i].style.left = selectedElements[0].offsetLeft + initialXOffsets[i] - 1 + 'px';
          }
          break;
        case "ArrowRight":
          event.preventDefault();
        ...
        case "ArrowUp":
          event.preventDefault();
          for (let i = 1; i < selectedElements.length; i++) {
            selectedElements[i].style.top = selectedElements[0].offsetTop + initialYOffsets[i] - 1 + 'px';
          }
          break;
        case "ArrowDown":
          event.preventDefault();
          ...
          break;
        case "Delete":
          console.log("in delete multi");
          // selectedElements[0] removal handled in Delete for single elements
          for (let i = 1; i < selectedElements.length; i++) {
